...
class Worker(request: PathfinderBrain.FindPath) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case Some(results: RunResults) =>
      val points = results.path.path.map { case Point(y, x) => PathPoint(y, x) }
      log.info("Successful pathfinding: {} {}", points, results.message)
...
//      Position("#1",2.4868245124816895,1.896909236907959,0.5,32.9348511589105),
